...
class ModelPlots(ModelStatistics):
    def __init__(self,
        ...
        (self.X_train,
         self.X_val,
         self.y_train,
         self.y_val) = train_test_split(self.X_train,
                                        self.y_train,
                                        test_size=0.1,
                                        random_state=42)
    ...
    def plot_results():
        ...
        plt.subplot(1,3,1)
        ax = sns.lineplot(y=np.array(train_loss),
                          x=np.array(epoch),
                          label="loss",
                          palette="binary")
        ...
        ax = sns.lineplot(x=np.array(epoch),
                          y=np.array(validation_loss),
                          label="Loss Convergence",
                          color="red")
        
        ax.set(xlabel='epochs',
               ylabel='validation_loss')
        
        plt.title("Train Vs Validation")
        #----------------------------------------------------------------
        plt.subplot(1,3,3)
        absolut_loss = np.subtract(validation_loss,
                                   train_loss)
